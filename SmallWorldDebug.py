...
class SmallWorldDebug(BaseMaskGenerator):
    def __init__(self, seq_len, device, grid_shape=None, distance_type='manhattan',
        ...
        self.seq_len = seq_len # Will be updated by patching.py if 0 initially
        self.device = device   # Will be updated by patching.py
        ...
        self.fixed_shape = None # Default to None
        if grid_shape:
            self.H, self.W = grid_shape
            self.fixed_shape = (self.H, self.W) # Store if provided
        elif seq_len > 0: # If seq_len is valid, calculate H,W now
            self.H = int((seq_len - 1) ** 0.5)
            self.W = self.H
            assert self.H * self.W + 1 == seq_len, "seq_len must be H*W+1 for grid + CLS"
        else: # seq_len is 0 or invalid, H,W will be set by _rebuild_grid later
            self.H, self.W = 0, 0 

        # self.coords will be initialized by _rebuild_grid in the first get_mask call
        # if H,W are valid at that point.
    ...
    def get_mask(self):
        # This method is called after patching.py updates self.seq_len and self.device.
        # _rebuild_grid ensures self.H, self.W, and self.coords are consistent with current self.seq_len.
        self._rebuild_grid()

        # Parameters that define the mask structure after _rebuild_grid has updated H, W, coords.
        current_params_for_mask_logic = (
            self.seq_len, 
            self.device.type, # Compare device type to avoid issues with different device objects
            self.H, 
            self.W, 
            self.radius,
            self.shortcut_prob,
        ...
        if self._cached_mask is not None and \
           ...
           self._cached_mask_params == current_params_for_mask_logic:
            if self.verbose:
                print(f"[Debug ({self.H}x{self.W},R{self.radius},P{self.shortcut_prob})] Returning cached mask.")
            mask = self._cached_mask
            return mask

        if self.verbose:
            print(f"[Debug ({self.H}x{self.W},R{self.radius},P{self.shortcut_prob})] Recomputing mask matrix.")

        N = self.seq_len
        mask_tensor = torch.zeros((N, N), dtype=torch.bool, device=self.device)

        # Local connections (between patch tokens, indices 1 to N-1)
        if N > 1 and hasattr(self, 'coords'): # Ensure coords exist and there are patch tokens
        ...
        if self.verbose:
            ...
            patch_local_cnt = mask_tensor[1:N, 1:N].sum().item()
        ...
        if N > 0: # Ensure mask is not empty
            mask_tensor[0, :] = True
            mask_tensor[:, 0] = True
        
        if self.verbose:
            total_cnt = mask_tensor.sum().item()
            print(f"[Debug ({self.H}x{self.W},R{self.radius},P{self.shortcut_prob})] Total connections in final mask (incl. CLS): {total_cnt}")

        self.mask = mask_tensor # Keep for potential direct access, though _cached_mask is primary
...
if __name__ == "__main__":
    ...
    parser.add_argument('--radius', type=int, default=6, help='Radius for mask generator')
